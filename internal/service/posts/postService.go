...
func (s *postService) CreateComment(comment models.Comment) error {
	...
	user, err := s.postRepo.GetUserByID(comment.AuthorID) // Метод получения пользователя по ID
	if err != nil {
...
func (s *postService) AddReaction(reaction models.Reaction) error {
	...
	user, err := s.postRepo.GetUserByID(reaction.UserID) // Метод получения пользователя по ID
	if err != nil {
